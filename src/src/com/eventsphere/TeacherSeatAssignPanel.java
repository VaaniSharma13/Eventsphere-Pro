...
public class TeacherSeatAssignPanel extends JPanel {
    private JComboBox<String> eventBox;
    private JTable seatTable;
    private String[] eventIds;
    private JButton updateBtn;

    public TeacherSeatAssignPanel(String username) {
        setLayout(new BorderLayout(10,10));
        setBackground(new Color(245,245,255));

        JLabel heading = new JLabel("Exam Seat Allotment", SwingConstants.CENTER);
    ...
    private void fillExams(String username) {
        try (Connection conn = DriverManager.getConnection(
            ...
            ResultSet rs = st.executeQuery(
                    "SELECT id, title FROM events WHERE created_by=" + tid + " AND event_type='EXAM' ORDER BY event_date DESC"
            );
            ...
            ex.printStackTrace();
            JOptionPane.showMessageDialog(this, "Error loading exam schedules: " + ex.getMessage());
        }
    }

    // Load registered students and current seat numbers
    private void loadSeats() {
        ...
        try (Connection conn = DriverManager.getConnection(
            ...
            while (rs.next()) {
                ...
                row.add(rs.getInt(3)); // registration id (hidden)
                data.add(row);
            }
